...
def get_separated_coor(img_path, gt_img):
    picture = np.array(Image.open(img_path).convert("L"), dtype=float)
    picture /= 255
    ylist, xlist = find_tfl_lights(picture, None) # here we change
    true_list = []
    false_list = []
...
def save_bin(data_list_all, lable_list_all, second_dir):
    np.array(data_list_all).tofile('./Data_dir/' + second_dir + '/data_main.bin')
    np.array(lable_list_all).astype('uint8').tofile('./Data_dir/' + second_dir + '/labels_main.bin')
