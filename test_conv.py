...
def test_conv():
    ...
    # image1 = ndimage.filters.convolve(image, filter_kernel, mode='constant', cval=0.0)
    image1 = sg.convolve2d(image, filter_kernel)
    fig = plt.figure()
    plt.gray()
    ...
    image = ndimage.filters.maximum_filter(image1, size=10)
    ax1.imshow(image)
    ax2.imshow(image1)
